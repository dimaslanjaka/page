...
class ExpressRollup {
	...
	handle(req, res, next) {
		...
		const bundlePath = join(
			root,
			src,
			pathname.replace(new RegExp(`^${dest}`), '').replace(extRegex, opts.bundleExtension),
		);
		...
		fsp
			.access(bundlePath, fsp.constants.R_OK)
			...
			.then(
				...
				err => {
					if (err.syscall && err.syscall === 'access') {
						this.log(
							'Bundle file not found. Since you might intend to serve this file statically, this is a silent warning.',
						);
					} else {
						console.error(err);
					}
	...
	processBundle(bundle, bundleOpts, dest, res, next, opts) {
		...
		bundle.generate(bundleOpts).then(bundled => {
			...
			if (opts.serve === true || opts.serve === 'on-compile') {
				...
				this.log('Serving', 'ourselves');
				res.status(200).type(opts.type).set('Cache-Control', `max-age=${opts.maxAge}`).send(bundled.code);
			} else {
				writePromise.then(
					() => {
	...
	allFilesOlder(file, files) {
		const statsPromises = [file].concat(files).map(f =>
			fsp.stat(f).then(
				stat => stat,
				() => false,
			),
	...
	checkNeedsRebuild(jsPath, rollupOpts) {
		...
		return testExists.then(() => this.allFilesOlder(jsPath, cache[jsPath])).then(allOlder => ({ needed: !allOlder }));
	}

	static getBundleDependencies(bundle) {
		return (bundle.modules || bundle.cache.modules).map(module => module.id).filter(path.isAbsolute);
	}
}
...
module.exports = function createExpressRollup(options) {
	...
	function middleware() {
		expressRollup.handle.apply(expressRollup, arguments);
	}
	return middleware;
};
